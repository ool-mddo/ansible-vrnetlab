...
cp playbooks/configs/Seg*conf $DEMO_DIR/emulated_asis/configs/
...
curl -X 'POST'   'http://localhost:1880/batfish_l1topo'   -H 'accept: application/json'   -H 'Content-Type: application/json'  --data @$DEMO_DIR/original_asis/topology.json > $DEMO_DIR/emulated_asis/batfish/layer1_topology.json
...
#DIR=$LOCALSERVER_HOMEDIR/clab-$LABNAME; find  $DIR/*/config  -name juniper.conf | xargs -IFILE bash -c "cat FILE"
...
cd $PLAYGROUND_DIR
sudo docker-compose run netomox-exp bundle exec rake NETWORK=${NETWORK_NAME} PHY_SS_ONLY=1 --trace
